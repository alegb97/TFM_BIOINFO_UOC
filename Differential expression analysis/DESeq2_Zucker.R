...
output_dir <- "GSEA/Zucker/DESeq2"
dir.create(output_dir, recursive = TRUE, showWarnings = FALSE)

# PASO 1: Cargar datos de featureCounts
counts <- read.csv("featureCounts_Zucker_results.csv", row.names = 1)

# Nombres muestras 
samples <- c("Zucker_S1_sorted.bam", "Zucker_S2_sorted.bam", "Zucker_S3_sorted.bam", 
                 "Zucker_S4_sorted.bam", "Zucker_S5_sorted.bam", "Zucker_S6_sorted.bam",
                 "Zucker_S7_sorted.bam", "Zucker_S8_sorted.bam", "Zucker_S9_sorted.bam", 
                 "Zucker_S10_sorted.bam", "Zucker_S11_sorted.bam", "Zucker_S12_sorted.bam", 
                 "Zucker_S13_sorted.bam","Zucker_S14_sorted.bam", "Zucker_S15_sorted.bam", "Zucker_S16_sorted.bam")

counts <- counts[, samples]

# Remover "_sorted.bam" de los nombres de las muestras
sample_names <- gsub("_sorted.bam", "", samples)
colnames(counts) <- sample_names

# PASO 2: Definir el diseño experimental
colData <- data.frame(
  row.names = sample_names,
  condition = c(rep("CT-GEN", 8), rep("OBS-GEN", 8))
)

# Convertir a factor
colData$condition <- factor(colData$condition, levels = c("CT-GEN", "OBS-GEN"))
...
res <- results(dds, contrast = c("condition", "OBS-GEN", "CT-GEN"))
...
ggplot(as.data.frame(res), aes(x=log2FoldChange, y=-log10(padj), color=color)) +
  ...
  ggtitle("Volcano Plot: CT-GEN vs OBS-GEN") +
...
pcaData$condition <- factor(pcaData$condition, levels = c("CT-GEN", "OBS-GEN"))
