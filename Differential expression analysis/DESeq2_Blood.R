...
counts_ordenado <- counts %>% arrange(Gene)


# Calcular la media de los valores duplicados
counts_sin_duplicados <- counts %>%
  group_by(Gene) %>%
  summarise(across(where(is.numeric), \(x) mean(x, na.rm = TRUE)), .groups = 'drop')

counts<-as.data.frame(counts_sin_duplicados)

rownames(counts)<-counts[,1]

# Filtramos las muestras de personas con peso normal y obesas con diabetes
samples <- c("Control1","Control2", "Control3", "Control4","Control5","Control6","Control7","Control8","Control9","Control10","Control11","Control12","Control13","Control14","Control15",
                  "O-T2DM1","O-T2DM2","O-T2DM3","O-T2DM4","O-T2DM5","O-T2DM6","O-T2DM7","O-T2DM8","O-T2DM9","O-T2DM10","O-T2DM11","O-T2DM12","O-T2DM13","O-T2DM14","O-T2DM15")

counts <- counts[, samples]

# Nombres de las muestras
sample_names <- c("Control1","Control2", "Control3", "Control4","Control5","Control6","Control7","Control8","Control9","Control10","Control11","Control12","Control13","Control14","Control15",
                  "O-T2DM1","O-T2DM2","O-T2DM3","O-T2DM4","O-T2DM5","O-T2DM6","O-T2DM7","O-T2DM8","O-T2DM9","O-T2DM10","O-T2DM11","O-T2DM12","O-T2DM13","O-T2DM14","O-T2DM15")
...
colData <- data.frame(
  ...
  condition = c(rep("Control", 15), rep("OBS-T2DM2", 15))
)

# Convertir a factor
colData$condition <- factor(colData$condition, levels = c("Control", "OBS-T2DM2"))
...
res <- results(dds, contrast = c("condition", "OBS-T2DM2", "Control"))
...
ggplot(as.data.frame(res), aes(x=log2FoldChange, y=-log10(padj), color=color)) +
  ...
  ggtitle("Volcano Plot: Control vs OBS-T2DM2") +
...
pcaData$condition <- factor(pcaData$condition, levels = c("Control", "OBS-T2DM2"))
